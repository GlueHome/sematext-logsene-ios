...
class Worker {
    ...
    private let timeTriggerSec: Double = 60
    ...
    init(client: LogseneClient, type: String, maxOfflineMessages: Int) throws {
        self.client = client
        self.type = type
        serialQueue = DispatchQueue(label: "logworker_events")
        ...
        timer = DispatchSource.makeTimerSource(flags: [], queue: serialQueue)
        timer.schedule(deadline: .now(), repeating: timeTriggerSec, leeway: .seconds(1))
    ...
    private func handleNewEvent(_ event: JsonObject) throws {
        try preflightBuffer.add(obj: event)
    ...
    private func sendInBatches() throws {
        while preflightBuffer.count > 0 {
          let batch = try preflightBuffer.peek(max: maxBatchSize)
            if sendBatch(batch) {
                invalidateTimer()
    ...
    private func sendBatch(_ batch: [JsonObject]) -> Bool {
    ...
    private func attemptExecute(_ bulkIndex: BulkIndex, attempts: Int) -> Bool {
